...
function Pagination({ pages }) {
  ...
  return (
    <div className="flex text-indigo-900 font-bold my-8">
      {page < 4 ? "" : (<Link key="firstpage" to={`page/${pages}`} className="p-5">Primeira</Link>)}
      {page === 1 ? "" : (<Link key="prevpage" to={`page/${page-1}`} className="p-5">&lt;</Link>)}
      {links}
      {page === pages ? "" : (<Link key="nextpage" to={`page/${page+1}`} className="p-5">&gt;</Link>)}
      {page === pages ? "" : (<Link key="lastpage" to={`page/${pages}`} className="p-5">Última</Link>)}
    </div>
  );
}
