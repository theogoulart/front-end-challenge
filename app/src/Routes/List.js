...
function List() {
  ...
  const [filters, setFilters] = useState(query.get("genres")?.split(',') || []);
  ...
  const filterHandler = (option, filters) => {
    ...
    navigate(`/?genres=${newFilters.sort().join(',')}`);
    return newFilters;
  };
    ...
    <div className="">
      <section className="flex flex-col md:items-center bg-purple-custom text-white p-4 py-14 md:p-20">
        ...
        <div className="flex flex-wrap md:justify-center items-center no-wrap max-w-screen-md">
          {genres.map(g => <FilterButton key={`genre-${g.id}`} clickHandler={() => {setFilters(filterHandler(g.name.toLowerCase(), filters))}} isSelected={filters.includes(g.name.toLowerCase())} genre={g.name} />)}
        </div>
      </section>
